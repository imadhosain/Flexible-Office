          ...
          <div class="d-flex flex-col container py-ma-mb-5">
            <form action="/addcontract" method="POST" enctype="multipart/form-data">
              ...
              <div class="form-group">
                ...
                <select class="form-control form-control-lg" name="officename">
                  @foreach ($office as $o)
                  <option value="{{ $o->officename }}" class="form-control">{{ $o->officename }}</option>
                  @endforeach
                </select>
              </div>
              <div class="form-group">
                <label class="text-muted h4 mb-1"><small>Customer</small></label>
                <select class="form-control form-control-lg" name="customername">
                  @foreach ($customer as $c)
                  <option value="{{ $c->customername }}" class="form-control">{{ $c->customername }}</option>
                  @endforeach
                </select>
              </div>
              <div class="form-group">
